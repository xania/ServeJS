...
function updateImport(source, path) {
    // self.addEventListener('error', e => importShim.e = e.error);
    return new Promise<any>(async (resolve, reject) => {
        const replaced = source.replace(/^\/\/# sourceMappingURL\=(.*)/im, `//# sourceURL=${absoluteUrl(path)}`);
        const blobUrl = createBlob(replaced);
...
export default function hot(module, path) {

    let __module = { ...module };
    listenForUpdates(path).subscribe(async (change) => {
        const updated = await updateImport(change.source, path + ".hot");
        __module = { ...updated };
    })
    return proxy(() => __module);

    function proxy(getter) {
        let value = getter();
        ...
        let x = new Proxy(value, {
            get(_, name) {
                return proxy(property(getter, name))
            },
            apply(target, thisArg: any, argArray?: any) {
                let func = getter();
                let result = func.apply(x, argArray);

                // console.log( { func, x, result });
                return result;
            }
        });
        return x;
    ...
    function listenForUpdates(path: string) {{
        ...
        return {
            subscribe(observer: (change: { source: string, path: string } ) => any ) {
